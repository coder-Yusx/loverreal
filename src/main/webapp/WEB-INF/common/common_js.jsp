<script type="text/javascript" src="${baseurl}/static/mould/assets/javascripts/jquery-1.8.3.min.js"></script>
<%-- <script type="text/javascript" src="${baseurl}/static/mould/assets/javascripts/jquery-ui-extras.min.js"></script>
<script type="text/javascript" src="${baseurl}/static/mould/assets/javascripts/jquery.mockjax.js"></script> --%>

<script type="text/javascript" src="${baseurl}/static/mould/assets/javascripts/bootstrap.min.js"></script>

<%-- <script type="text/javascript" src="${baseurl}/static/mould/assets/javascripts/pixel-admin.min.js"></script>
<script type="text/javascript" src="${baseurl}/static/mould/assets/javascripts/ie.min.js"></script>
 

<script type="text/javascript" src="${baseurl}/static/mould/assets/demo/demo.js"></script> 
<script type="text/javascript" src="${baseurl}/static/mould/assets/javascripts/demo-mock.js"></script> --%>